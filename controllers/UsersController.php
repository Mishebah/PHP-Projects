	 			...
	 			public function beforeAction($action)
{
  ...
  if (($model->load(Yii::$app->request->post()) && $model->validate()) && ($userGroupsmodel->load(Yii::$app->request->post()) && $userGroupsmodel->validate())) 
	...
	$model->createdBy= $userGroupsmodel->insertedBy= yii::$app->user->identity->userID;
	 ...
	 $model->status = $userGroupsmodel->status = StatusCodes::ACTIVE;
	...
	$model->uuid = Yii::$app->security->generateRandomString(8);
	$model->status = StatusCodes::INACTIVE;
...
//                ['html' => 'passwordResetToken-html', 'text' => 'passwordResetToken-text'],
    ...
    public function actionUpdate($id)
    {
        ...
        $model = $this->findModel($id);
		$userGroupsmodel =  UserGroups::find()->where(['userID'=>$id,'status'=>1])->one();
